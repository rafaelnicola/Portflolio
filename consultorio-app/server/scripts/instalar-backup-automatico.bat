...
REM Crea una tarea programada de Windows que corre backup.ps1 todos los domingos a las 22:00.
REM Hay que ejecutar este archivo como Administrador (clic derecho -> Ejecutar como administrador).

schtasks /Create /TN "BiomedicalCenter_BackupSemanal" /TR "powershell.exe -NoProfile -ExecutionPolicy Bypass -File \"%SCRIPT_DIR%backup.ps1\"" /SC WEEKLY /D SUN /ST 22:00 /RL HIGHEST /F

if %ERRORLEVEL% EQU 0 (
    echo.
    echo Listo. El backup se va a hacer automaticamente todos los domingos a las 22:00,
